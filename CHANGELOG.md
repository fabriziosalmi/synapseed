...
### Sentinel Hardening + CI Lockfile + README Cleanup

#### Command Sentinel Security Hardening (`crates/root/src/sentinel.rs`)
- **CRITICAL fix:** Shell chaining defense — commands are now split on `;`, `|`, `&&`, `||`,
  and newlines; each segment is evaluated independently. Previously `ls; rm -rf /` bypassed
  all deny rules because the `ls` allow-rule matched first.
- **New deny rules:** command substitution (`$()`, backticks), base64 decode, interpreter
  inline execution (`python -c`, `ruby -e`, `perl -e`, `node -e`), `nohup`
- **Enhanced:** chmod now blocks `0777`, `a+rwx`, `a=rwx` (was only literal `777`)
- **New:** null byte rejection (C-string truncation attacks)
- 24 new tests covering all chaining, substitution, and obfuscation vectors (73 total)

#### CI Lockfile Check (`.github/workflows/ci.yml`)
- Added `cargo check --locked --workspace` before build step
- Catches Cargo.lock drift before tests run

#### README Cleanup
- Replaced marketing tagline with factual description
- Feature table now describes capabilities without comparison framing

## [4.28.0] — 2026-02-16

### Metadata Fallback + VSCode Extension Fix

#### Metadata File Fallback (`crates/whisper/src/router/code.rs`)
- **New:** when CodeGraph has no symbol for a metadata file target (Cargo.toml, LICENSE, .toml),
  reads the file directly and injects a pseudo-symbol with extracted key-value facts
- Fixes q01_version benchmark (F1 0.0 → 1.0): version string now appears in smart_context
- `build_human_summary` now handles `Constant` kind with Content preview

#### Benchmark Suite v1.2 (`crates/bench/suites/grounding_v1.jsonl`)
- Ground truth updated to v4.28.0
- q04 expected_symbols fixed: `run_once` → `run_check` (matching actual function name)

#### VSCode Extension — Field Alignment + UX
- **Fix:** `ArchReport` interface now matches Rust serialized field names:
  `module_name`, `efferent_coupling`, `afferent_coupling`, `approx_complexity`
  (was `name`, `ce`, `ca`, `complexity` → caused "undefined" in sidebar)
- **Fix:** Violations use `rule`/`description` (was `kind`/`message`)
- **Fix:** Recommendations rendered as structured objects with priority, category, action
- **UX:** No-flash refresh across all 5 providers — keeps previous data visible during reload

## [4.27.0] — 2026-02-16

### Body Enrichment — Smart Context Quality Boost

#### Snippet Enrichment in Code Gatherer (`crates/whisper/src/router/code.rs`)
- **New:** `gather_code_context` injects a `snippet` field (up to 40 lines of symbol body) into symbol JSON
- Enables downstream context builder to extract structural details without raw injection

#### Enhanced `build_human_summary` (`crates/whisper/src/router/context_builder/mod.rs`)
- **Traits/Interfaces:** new `Methods:` line listing all method names (fixes q06 benchmark)
- **Structs/Classes:** new `Fields:` line listing all field names (fixes q07 benchmark)
- **Enums:** new `Variants:` line listing enum variants
- **Functions/Methods:** new `Body:` line with first 12 significant lines (fixes q04, q09 benchmark)
- 3 new extraction helpers: `extract_method_names`, `extract_field_names`, `extract_body_preview`
- 4 new unit tests covering all enrichment paths + backward compatibility

#### Updated Benchmark Suite (`crates/bench/suites/grounding_v1.jsonl`)
- Ground truth updated from v4.15.0 → v4.26.0 (version, tool count, methods, fields)

## [4.26.0] — 2026-02-16

### Audit Hardening — Reactor Safety & Sandbox Limits

#### IndexWriter Reactor Stall Fix (`crates/search/src/plugin.rs`)
- **Fix:** moved blocking Tantivy `IndexWriter` operations (`remove_file`, `reindex_file`) from async `on_event` to `std::thread::spawn`
- Prevents `std::sync::Mutex` from blocking the tokio reactor thread during file change events
- Fire-and-forget pattern: search index is eventually consistent

#### Gym Sandbox Resource Limits (`crates/gym/src/sandbox.rs`)
- **`CARGO_BUILD_JOBS=2`**: caps parallel rustc instances to limit CPU usage
- **`RUST_MIN_STACK=2097152`**: 2 MB stack per thread as stack overflow guard
- Documented memory/cgroup limitation (requires containers or `unsafe setrlimit`)

## [4.25.0] — 2026-02-16

### La Guardia — Security Docs & DLP Hardening

#### D39: Shadow Compiler build.rs Security (`docs/`, `crates/shadow-check/`)
- **New:** `docs/security/build-scripts.md` — comprehensive threat model, dangerous patterns, mitigations
- **DNA opt-out:** `hci.shadow_check: false` disables background `cargo check` for untrusted projects
- Updated `docs/security/model.md` Layer 4 with build.rs risk note
- Updated `docs/features/shadow-check.md` with correct DNA flag path

#### D22: DLP Concatenation Evasion Detection (`crates/husk/src/scanner.rs`)
- **Pass 2b:** new post-regex pass that merges string concatenation operators (`" + "`) before scanning
- Catches secrets split across adjacent string literals (e.g., `"postgres" + "ql://user:pass@host"`)
- Deduplication: findings already caught in Pass 2 are not double-reported
- Refactored regex scanning into reusable `scan_regex_pass()` method
- 3 new tests: concatenated URI credentials, concatenated generic secret, no false positive on innocent concat

## [4.24.0] — 2026-02-16

### Il Parallelo — Hardening & Parallel Dispatch

#### D27: Parallel Gather Dispatch (`crates/whisper/src/router/mod.rs`)
- **Stage 6 parallelized**: diagnostics, history, code, and security gatherers now run concurrently via `std::thread::scope`
- Four independent threads with graceful fallback on panic (`unwrap_or` defaults)
- No new dependencies — uses stdlib scoped threads (SynapseContext is `Send + Sync`)

#### D30: Blame AI Attribution (`crates/chronos/src/historian.rs`)
- **`BlameInfo.co_authors`**: new `Vec<String>` field extracted from `Co-authored-by:` / `Co-Authored-By:` trailers
- Enables tracking AI-generated quickfix modifications in git blame output
- Serialization: `#[serde(skip_serializing_if = "Vec::is_empty")]` — zero overhead when absent

#### D36: DLP Entropy Gate on Encoded Fragments (`crates/husk/src/scanner.rs`)
- **Pass 0 hardened**: Base64 and Hex decoded blobs now checked against Shannon entropy threshold (`min_entropy`)
- Low-entropy structured data (config, padding, UUIDs) dropped before pattern matching — reduces false positives
- `decode_encoded_fragments` promoted from static `fn` to instance method for entropy access

#### D40: SearchBackend Trait — Full Wiring (`crates/whisper/src/router/extraction/mod.rs`)
- **Pass 7 (Reverse Test Lookup)** now uses `&dyn SearchBackend` instead of concrete `SemanticIndex`
- Completes backend-agnostic abstraction started in Pass 2

## [4.23.0] — 2026-02-14

### Il Polso — Pulse Activity Counters

#### Pulse System (`crates/core/src/pulse.rs`)
- **PulseStore**: lock-free activity counter registry with exponential decay (half-life 8 min)
- **Two counters**: `file_touched` and `symbol_touched` — tracks which files and symbols the model interacts with
- **Exponential decay**: `weight = e^(-λ × age)` with λ = ln(2)/480 — recent activity dominates, old events fade naturally
- **Ring buffer**: 1024 events per counter key, FIFO eviction — bounded memory, O(1) insert
- **Context tiebreaker**: 10% pulse bonus blended into `inject_raw_sources` sort — never dominates search relevance, breaks ties toward working set
- **MCP Resource**: `synapseed://pulse` — exposes live activity snapshot (counters, scores, event counts) to clients
- **Producer**: Whisper router records file_touched/symbol_touched after every `ask` query
- **Consumer**: ContextBuilder uses pulse scores as tiebreaker when ranking source targets
- 9 unit tests covering decay math, ring buffer eviction, counter isolation, snapshot consistency

#### MCP
- Resource count: 12 → 13 (added `synapseed://pulse`)

## [4.22.0] — 2026-02-14

### Il Respiro — Benchmark Without Limits

#### Benchmark Suite
- **max_tokens**: removed 4096 hardcap → 0 (unlimited, omitted from API call entirely)
- **LLM timeout**: 120s → 900s (15 min per call) — local 4B models need room to think
- **max_retries**: 1 → 2 (more resilient dual-endpoint failover)
- **TASK_TIMEOUT**: replaced fixed 600s with dynamic formula: `(n_conditions + 2) × timeout × 1.5`
- Root cause: qwen3-4b spent all 4096 tokens inside `<think>` blocks, leaving `finish_reason: length` with empty answers
- With unlimited tokens, models complete their reasoning chain and produce real answers
- `.env`: `LLM_TIMEOUT=900`, `LLM_MAX_TOKENS` commented out (default 0 = unlimited)

## [4.21.0] — 2026-02-14

### Il Budgettiere — Context Budgeting Strategy v5.0

#### Context Budgeting
- **New tier: Universal** (>14B / Cloud models) — 32K token budget, holistic search strategy
- **Derived budgets**: all char budgets, critical pools, min-lines computed from a single `token_budget()` parameter
- **80% Rule**: ContextBuilder targets 80% of tier budget, reserving 20% headroom for system prompt + user query
- **Tier budget table**: Atomic 2K, Molecular 8K, Galactic 16K, Universal 32K tokens
- **Critical pool ratio**: scales inversely (Atomic 35% → Universal 20%) — small models see fewer but complete symbols
- Atomic source budget reduced from 32K chars → ~5.3K chars (eliminates over-context hallucinations)
- `inject_raw_sources` now takes `ModelTier` instead of `bool` — uses `tier.source_char_budget()` and `tier.critical_char_budget()`
- `needs_semantic_ballast()` and `max_unique_files()` replace hardcoded `tier == ModelTier::Atomic` checks

#### Coherence Gate
- Uses `tier.max_clusters()` (Atomic→1, Molecular→2, Galactic→3, Universal→5)
- Greedy pruning generalized to all tiers via `tier.max_unique_files()`

#### Client Fingerprinting
- Claude/GPT/Gemini → Universal (was Galactic)
- Cursor/Windsurf/Zed/Continue → Galactic (was Galactic)
- Added `continue` IDE detection

#### VS Code Extension
- **Benchmark Results Panel**: new webview (like Dashboard) for visualizing benchmark JSON results
- Auto-detects benchmark type (coding/grounding/search/engine) from filename and content
- Overview tab with summary cards, Coding tab with per-condition breakdowns and difficulty charts
- Expandable task details, import button, file click opens raw JSON
- Command: `SYNAPSEED: Open Benchmark Results` + cycle panel integration

## [4.20.0] — 2026-02-14

### La Lente Critica — Critical Symbol Registry & Dedicated Budget

Fixes the tool count truncation problem where `list_tools()` (24 tools) was
reduced to 13 by the 20+20 sandwich strategy. Three-layer fix:

- **DNA Config**: `critical_symbols` in `ContextConfig` (user-configurable list,
  defaults to `["list_tools", "list_resources"]`)
- **Indexer**: `extract_body_snippet` now uses compact registry format (signature +
  `// REGISTRY: N items — name1, name2, ...`) instead of sandwich truncation when
  ≥10 `name: "..."` entries are detected
- **Context Builder**: critical symbols get a dedicated budget pool (8K molecular /
  12K atomic) separate from the shared 16K/32K pool, preventing starvation of
  other symbols while guaranteeing complete registry injection

## [4.19.0] — 2026-02-15

### Il Libro Cognitivo — Cognitive Ledger & Operational Moment Classification

New **Cognitive Ledger** system extends the Flight Recorder with deterministic
Operational Moment classification. Infers the developer's cognitive state from
session metrics — no LLM calls, fully auditable.

#### Cognitive Ledger Core (`crates/core/src/ledger.rs` — 780+ LoC, 13 tests)
- **10 Operational Moments**: RapidScaffolding, DeepBackendLogic, ArchitectureWiring,
  FrontendImplementation, UxPolish, CrossLayerConsistency, IterativeDistress,
  MomentumFlow, ExploratoryRefactoring, EmergencyPatching.
- **SessionPulse**: Per-tool-call classification with needle range (1–10), mode suggestion
  (Momentum/Reasoning/DeepReasoning), layer label, and session weight.
- **Auditable Evidence**: Every classification includes the exact metric values that
  triggered it (tool call count, avg interval, file variance, loop repeat count, etc.).
- **Priority Cascade**: Emergency → Iterative Distress → Pattern-based → MomentumFlow.
- **MomentClassifier**: Pure function — classification is stateless and deterministic.

#### Session Hint Injection
- **`[SESSION_HINT]` breadcrumb**: Injected into `build_smart_context()` after language
  pinning. Gives the LLM situational awareness of the current focus module and cognitive
  phase without restricting search accuracy.
- Compendium philosophy: enhances context density, never conditions or filters results.

#### Recency Boost (hybrid search)
- Working-set files get up to **1.2× score boost** in hybrid search (RRF fusion).
- Exact file match → full 1.2× boost; symbol substring match → 1.1× boost.
- Never penalizes: boost is always ≥ 1.0 (compendium mode).
- Applied post-fusion in `extraction.rs`, preserving hybrid search purity.

#### Recorder Extensions
- `FlightRecorder::build_metrics_snapshot()`: Assembles a `MetricsSnapshot` from the
  working set for the moment classifier.
- `FlightRecorder::working_set_subjects()`: Returns active file subjects for recency boost.

#### New MCP Resource
- **`synapseed://session/context`**: Cognitive Ledger session pulse with moment, needle
  range, mode, layer, evidence, and session hint. 12 → 12 resources total.
- **`synapseed://context/active`**: Enhanced with `cognitive_ledger` field showing current
  moment classification alongside flight recorder data.
