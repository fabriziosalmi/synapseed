...
export class DiagnosticsProvider implements vscode.TreeDataProvider<SynapseedItem>, vscode.Disposable {
        ...
        try {
            ...
            for (const line of text.split('\n')) {
                ...
                if (fm) {
                    ...
                    const lineNum = Math.max(0, parseInt(fm[2]) - 1);
                    const col = Math.max(0, parseInt(fm[3] ?? '1') - 1);
                    item.command = {
                        command: 'vscode.open',
                        title: 'Go to Error',
            ...
            log.error('Diagnostics load failed', e);
