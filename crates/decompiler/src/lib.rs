...
/// Maximum binary file size: 100 MB. Files larger than this are rejected
/// to prevent out-of-memory conditions from crafted binaries (D80).
...
pub fn analyze_binary(path: &std::path::Path) -> Result<BinaryAnalysis> {
    // D80: Guard against OOM from oversized binaries
    let metadata = std::fs::metadata(path)?;
    if metadata.len() > MAX_BINARY_SIZE {
        anyhow::bail!(
            "Binary file too large ({:.1} MB, max {:.1} MB)",
            metadata.len() as f64 / (1024.0 * 1024.0),
            MAX_BINARY_SIZE as f64 / (1024.0 * 1024.0),
        );
    }
