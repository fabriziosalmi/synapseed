...
pub struct HciConfig {
    ...
    /// D39: Enable background `cargo check` (Shadow Compiler).
    /// Set to `false` to disable — recommended for untrusted projects where
    /// `build.rs` scripts may execute arbitrary code.
    #[serde(default = "default_true")]
...
pub struct ContextConfig {
    ...
    /// Symbol names whose body must never be truncated during context injection.
    /// Use for registry functions (tool lists, route tables) where truncation
    /// causes the model to lose critical entries and miscount items.
    /// Default: ["list_tools", "list_resources"]
    #[serde(default = "default_critical_symbols")]
    pub critical_symbols: Vec<String>,

    /// Pulse blend weight (0.0–1.0) for context ranking tiebreaker.
    /// Controls how much recent-activity signal influences source injection order.
    /// 0.0 = pulse disabled, 1.0 = pulse dominates. Default: 0.25.
    #[serde(default = "default_pulse_blend_weight")]
    pub pulse_blend_weight: f32,
}
...
impl Default for ContextConfig {
    fn default() -> Self {
        Self {
            ...
            critical_symbols: default_critical_symbols(),
            pulse_blend_weight: default_pulse_blend_weight(),
        }
    }
}
...
fn default_critical_symbols() -> Vec<String> {
    vec!["list_tools".into(), "list_resources".into()]
}

fn default_pulse_blend_weight() -> f32 {
    0.25
}
...
impl ProjectDna {
    ...
    fn merge(&mut self, other: Self) {
        ...
        if other.context.critical_symbols != default_critical_symbols() {
            self.context.critical_symbols = other.context.critical_symbols;
        }
    }
}
