...
pub struct WorkingSetEntry {
    /// Unique monotonic ID for causal tracking.
    pub entry_id: u64,
...
pub enum EventKind {
    ...
    SearchQuery,
    /// Auto-fix proposed by RepairOrchestrator
    AutoFixProposed,
    /// Auto-fix applied (or reverted)
    AutoFixApplied,
}
...
/// A detected causal relationship between two events.
///
/// Built automatically by the recorder when temporal proximity and event-type
/// patterns indicate a cause→effect chain.  Example:
/// `FileChanged(“src/lib.rs”) → [340ms] → Diagnostic(“5 errors”) → [120ms] → AutoFixProposed(“E0425”)`
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CausalLink {
    /// Entry ID of the cause event.
    pub cause_id: u64,
    /// Entry ID of the effect event.
    pub effect_id: u64,
    /// Kind of the cause event.
    pub cause_kind: EventKind,
    /// Kind of the effect event.
    pub effect_kind: EventKind,
    /// Subject of the cause event.
    pub cause_subject: String,
    /// Subject of the effect event.
    pub effect_subject: String,
    /// Latency between cause and effect in milliseconds.
    pub latency_ms: u64,
}
...
pub struct FlightRecorder {
    ...
    /// Monotonic entry ID counter for causal tracking.
    next_entry_id: u64,
    /// Detected causal chain linking events by cause→effect.
...
impl ActivityCounters {
    ...
    fn record(&mut self, kind: EventKind, tool_name: Option<&str>) {
        match kind {
            ...
            EventKind::AutoFixProposed | EventKind::AutoFixApplied => self.fixes += 1,
            EventKind::ToolCall => {
...
impl FlightRecorder {
    ...
    pub fn record(
        ...
        // ── Causal Tracking: detect cause→effect links ─────────────
        self.detect_causal_link(&entry);
    ...
    /// Build a [`MetricsSnapshot`] for the Cognitive Ledger's moment classifier.
    ///
    /// Extracts the last N events from the working set into the format
    /// expected by [`crate::ledger::MomentClassifier::classify`].
    pub fn build_metrics_snapshot(
        ...
        if let Some(phase) = self.current_phase() {
            snap.current_activity = phase.activity.to_string();
            snap.focus_module = phase.module.clone();
        }

        // Extract from working set (newest last in VecDeque, we want newest first)
        for entry in self.working_set.iter().rev().take(window) {
            if let Some(inst) = entry.instant {
                snap.call_timestamps.push_back(inst);
            }
            // Tool name from subject for ToolCall events
            let tool = if entry.kind == EventKind::ToolCall {
                entry.subject.clone()
            } else {
                format!("{:?}", entry.kind).to_lowercase()
            };
            snap.call_tools.push_back(tool);
            snap.call_subjects.push_back(entry.subject.clone());
            snap.call_modules.push_back(entry.module.clone());
        }
    ...
    pub fn causal_chain(&self) -> &[CausalLink] {
        &self.causal_chain
    }

    // ── Causal Link Detection ──────────────────────────────────────

    /// Maximum time window for detecting causal relationships.
    const CAUSAL_WINDOW_SECS: u64 = 10;

    /// Maximum causal links kept in memory.
    const MAX_CAUSAL_LINKS: usize = 100;

    /// Detect causal links between the new event and recent prior events.
    ///
    /// Uses temporal proximity + event-type heuristics:
    /// - `FileChange` → `Diagnostic`  (file save caused compiler errors)
    /// - `Diagnostic`  → `AutoFixProposed` (errors triggered repair proposal)
    /// - `AutoFixProposed` → `AutoFixApplied` (proposal was approved/applied)
    fn detect_causal_link(&mut self, new_entry: &WorkingSetEntry) {
        let cause_kind = match new_entry.kind {
            EventKind::Diagnostic => Some(EventKind::FileChange),
            EventKind::AutoFixProposed => Some(EventKind::Diagnostic),
            EventKind::AutoFixApplied => Some(EventKind::AutoFixProposed),
            _ => None,
        };

        let Some(cause_kind) = cause_kind else { return };
        let Some(now) = new_entry.instant else { return };
        let causal_window = Duration::from_secs(Self::CAUSAL_WINDOW_SECS);

        // Scan working set (most recent first) for matching cause
        for prev in self.working_set.iter().rev() {
            ...
            let link = CausalLink {
                cause_id: prev.entry_id,
                effect_id: new_entry.entry_id,
                cause_kind,
                effect_kind: new_entry.kind,
                cause_subject: prev.subject.clone(),
                effect_subject: new_entry.subject.clone(),
                latency_ms: now.duration_since(prev_instant).as_millis() as u64,
            };

            if self.causal_chain.len() >= Self::MAX_CAUSAL_LINKS {
                self.causal_chain.remove(0);
            }
            self.causal_chain.push(link);
            break; // Only link to the most recent matching cause
        }
    }
