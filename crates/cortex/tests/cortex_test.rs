...
// ══════════════════════════════════════════════════════════════
// D14: remove_file — ghost symbol pruning
// ══════════════════════════════════════════════════════════════

#[test]
fn test_remove_file_clears_all_indices() {
    let dir = TempDir::new().unwrap();
    write_file(
        dir.path(),
        "alpha.rs",
    ...
    assert!(!graph.lookup("AlphaStruct").is_empty());
    assert!(!graph.lookup("beta_fn").is_empty());
...
fn test_remove_file_nonexistent_is_noop() {
    let graph = CodeGraph::new();
    let removed = graph.remove_file(Path::new("/nonexistent/file.rs"));
    assert_eq!(removed, 0);
    assert_eq!(graph.file_count(), 0);
}
...
fn test_remove_then_reindex() {
    let dir = TempDir::new().unwrap();
    write_file(dir.path(), "module.rs", "pub fn original() {}\n");
    ...
    graph.remove_file(&dir.path().join("module.rs"));
    assert!(graph.lookup("original").is_empty());
