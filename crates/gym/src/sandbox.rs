...
impl Sandbox {
    ...
    pub fn inject(&self, scenario: &Scenario) -> Result<(), crate::GymError> {
        // D54: Validate source code doesn't contain sandbox-escape patterns
        validate_source_safety(&scenario.source_code)?;
        if !scenario.test_code.is_empty() {
            validate_source_safety(&scenario.test_code)?;
        }
    ...
    ///
    /// D89: Uses preemptive `Child::kill()` — if the subprocess exceeds
    /// `timeout_secs`, it is SIGKILL'd immediately rather than checked
    /// post-hoc.  Prevents infinite-loop build scripts from hanging the
    /// MCP server thread.
    pub fn build(&self, timeout_secs: u64) -> Result<(CompilationResult, u64), crate::GymError> {
        let start = Instant::now();

        let output = run_with_preemptive_timeout(
            Command::new("cargo")
    ...
    ///
    /// D89: Uses preemptive `Child::kill()` — same as `build()`.
...
/// D54: Validate that user-submitted source code doesn't contain
/// dangerous patterns that could escape the sandbox.
fn validate_source_safety(source: &str) -> Result<(), crate::GymError> {
    const FORBIDDEN_PATTERNS: &[(&str, &str)] = &[
        ("std::process", "process spawning (sandbox escape)"),
        ("Command::new", "command execution (sandbox escape)"),
        ("std::net", "runtime network access"),
        ("std::os::unix", "Unix raw operations"),
        ("std::os::windows", "Windows raw operations"),
        ("libc::", "libc FFI"),
        ("nix::", "nix FFI"),
        ("include_str!", "file inclusion"),
        ("include_bytes!", "file inclusion"),
        ("extern \"C\"", "C FFI declarations"),
        ("extern \"system\"", "system FFI declarations"),
    ];
...
// ── D89: Preemptive timeout helper ─────────────────────────────────────

/// Spawn a child process and kill it if it exceeds `timeout_secs`.
///
/// Uses `try_wait()` polling + background pipe-reader threads to avoid
/// pipe-buffer deadlocks. No external crate dependency.
///
/// Resource limits (audit hardening):
/// - `CARGO_BUILD_JOBS=2`: caps parallel rustc instances (CPU)
/// - `RUST_MIN_STACK=2097152`: 2 MB stack per thread (stack overflow guard)
/// - Preemptive SIGKILL on timeout (D89)
/// - Note: memory/cgroup limits require `setrlimit` (unsafe) or containers;
///   the timeout covers most OOM scenarios since the process will be killed.
fn run_with_preemptive_timeout(
    cmd: &mut Command,
    timeout_secs: u64,
) -> Result<std::process::Output, crate::GymError> {
    use std::io::Read;

    let mut child = cmd
        // Audit hardening: cap CPU parallelism and stack size.
        .env("CARGO_BUILD_JOBS", "2")
        .env("RUST_MIN_STACK", "2097152")
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped())
        .spawn()
        .map_err(|e| crate::GymError::Build(format!("Failed to spawn: {e}")))?;
    ...
    // Poll child with 250ms granularity.
        ...
        match child.try_wait() {
            ...
            Ok(None) => {
                if start.elapsed() > deadline {
                    warn!(timeout_secs, "Gym: child exceeded timeout, sending SIGKILL");
                    let _ = child.kill();
            ...
            Err(e) => return Err(crate::GymError::Build(format!("try_wait failed: {e}"))),
        }
    };
