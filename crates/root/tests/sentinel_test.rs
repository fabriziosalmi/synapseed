...
fn deny_newline_injection() {
    // A command that embeds a newline to try to sneak in a second command.
    // "ls\nrm -rf /" -- after trim, the regex checks trimmed string.
    // The rm -rf / pattern should match since the full string is checked.
    let sentinel = default_sentinel();
    let cmd = "ls\nrm -rf /";
    // This could go either way depending on regex behavior with \n.
    // But "ls" will match first (Allow), so the command is allowed.
    // This documents the current behavior: first-match wins.
    let result = sentinel.evaluate(cmd);
    // The "ls" at the start matches the allow rule first.
    assert!(result.is_ok(), "First-match-wins: 'ls' prefix matched Allow");
}
