...
impl SynapsePlugin for SearchPlugin {
        ...
        Box::pin(async move {
            if let SynapseEvent::FileChanged { path, kind } = event {
                if let Some(index) = &self.index {
                    ...
                    // Audit finding: IndexWriter uses std::sync::Mutex internally.
                    // Acquiring it from an async context blocks the reactor thread.
                    // Spawn a native thread for the blocking Tantivy write operations.
                    ...
                    std::thread::spawn(move || match kind {
                        FileChangeKind::Deleted => {
                            ...
                            #[cfg(feature = "embeddings")]
                            if let Some(vi) =
                                ctx_clone.get_extension::<crate::vector_index::VectorIndex>()
                            {
                                vi.remove_file(&path);
                                debug!(file = %path, "Search: Removed from vector index");
                            }
                            let _ = ctx_clone; // suppress unused warning when feature disabled
                        }
                            ...
                            if let Ok(mut parser) = AstParser::new() {
                                if let Ok(source) = std::fs::read_to_string(file_path) {
                                        ...
                                        parser.parse_file(file_path, &source)
                                    {
                                        ...
                                        #[cfg(feature = "embeddings")]
                                        reembed_file(&file_structure, &ctx_clone);
                                    }
                                }
                            }
                            let _ = ctx_clone; // suppress unused warning when feature disabled
    ...
    fn priority(&self) -> u32 {
        200 // After cortex (100)
    }
}

/// Build the embedding text for a symbol (v5.0.1: structured budget allocation).
///
/// Weighted concatenation: Name (3x) | Signature (2x) | Docstring (1x) | Body Keywords (0.5x).
/// Uses structured budget allocation to prevent name/signature from starving
/// body keywords — the body is often the only signal that distinguishes two
...
fn build_embedding_text(sym: &synapseed_core::symbol::Symbol, source: Option<&str>) -> String {
    ...
    let name_part = format!("{0} {0} {0}", sym.name);
    let name_truncated = if name_part.len() > NAME_BUDGET {
        &name_part[..name_part.floor_char_boundary(NAME_BUDGET)]
    } else {
        &name_part
    };
    text.push_str(name_truncated);

    // Signature (weight: 2x)
    if let Some(sig) = &sym.signature {
        let sig_part = format!(" {0} {0}", sig);
        let sig_truncated = if sig_part.len() > SIG_BUDGET {
            &sig_part[..sig_part.floor_char_boundary(SIG_BUDGET)]
        } else {
            &sig_part
        };
        text.push_str(sig_truncated);
    }
    ...
    if let Some(src) = source {
        ...
        if !doc.is_empty() {
            let space_doc = format!(" {}", doc);
            let remaining_for_doc = DOC_BUDGET.min(TOTAL_BUDGET.saturating_sub(text.len()));
            if remaining_for_doc > 10 {
                let doc_truncated = if space_doc.len() > remaining_for_doc {
                    &space_doc[..space_doc.floor_char_boundary(remaining_for_doc)]
                } else {
                    &space_doc
                };
    ...
    // Body keywords (weight: 0.5x — gets ALL remaining budget)
    if let Some(src) = source {
        let body_kw = extract_body_keywords(src, sym.line_start, sym.line_end, 10);
        if !body_kw.is_empty() {
            let remaining = TOTAL_BUDGET.saturating_sub(text.len());
            if remaining > 10 {
                let space_kw = format!(" {}", body_kw);
                let kw_truncated = if space_kw.len() > remaining {
                    &space_kw[..space_kw.floor_char_boundary(remaining)]
                } else {
                    &space_kw
                };
                text.push_str(kw_truncated);
            }
        }
    }
