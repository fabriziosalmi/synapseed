...
pub enum ProposalCategory {
    ...
    UnusedDependency,
    /// Compiler error with MachineApplicable suggestion (Action Chaining).
    CompilerError,
    /// Auto-repair proposal from the RepairOrchestrator.
    AutoRepair,
}
...
impl Proposal {
    ...
    /// Human-readable label for the proposal category.
    pub fn category_label(&self) -> &str {
        match self.category {
            ProposalCategory::Clippy => "clippy",
            ProposalCategory::CompilerWarning => "compiler-warning",
            ProposalCategory::UnusedDependency => "unused-dep",
            ProposalCategory::CompilerError => "compiler-error",
            ProposalCategory::AutoRepair => "auto-repair",
        }
    }
}
...
/// D48: Maximum number of proposals to prevent unbounded memory growth.
...
impl ProposalStore {
    ...
    ///
    /// D48: Evicts resolved (applied/rejected) proposals when the store
    /// exceeds `MAX_PROPOSALS`, preventing unbounded memory growth.
    pub fn add(&self, proposal: Proposal) {
        if self.proposals.len() >= MAX_PROPOSALS {
            // First pass: remove applied/rejected proposals
