...
// ── D22: String Concatenation Evasion Detection ─────────────────────
...
fn detect_concatenated_uri_credentials() {
    ...
    assert!(
        result.is_err(),
        "URI credentials split across concatenated strings should be detected"
    );
}

#[test]
fn detect_concatenated_generic_secret() {
    let guard = SecurityGuard::with_defaults();
    let content = r#"let key = "password" + "=" + "supersecretvalue123";"#;
    let result = guard.check(content);
    assert!(
        result.is_err(),
        "Generic secret split across concatenated strings should be detected"
    );
}

#[test]
fn no_false_positive_normal_string_concat() {
    let guard = SecurityGuard::with_defaults();
    // Normal string concatenation with no secrets
    let content = r#"let msg = "hello" + " " + "world";"#;
    let result = guard.check(content);
    assert!(
        result.is_ok(),
        "Innocent string concatenation should not trigger DLP"
    );
}
